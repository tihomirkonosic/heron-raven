...
namespace raven {
                    ...
                    for (auto &ovlp : ovlps) {
                      if (overlap_length(ovlp) > 500) {
                        ...
                        ovlp.lhs_end =
                            ...
                            ? ovlp.lhs_end + (param.window_len + param.kmer_len - 1)
                            : sequences[ovlp.lhs_id]->inflated_len;
                        ...
                        ovlp.rhs_end =
                            ...
                            ? ovlp.rhs_end + (param.window_len + param.kmer_len - 1)
                            : sequences[ovlp.rhs_id]->inflated_len;
  ...
  void Graph_Constructor::LoadOverlaps(const std::string &overlaps_path,
    ...
    auto get_read_id =
        [](const std::string &read_name, std::vector<std::unique_ptr<biosoup::NucleicAcid>> &sequences) -> std::uint32_t {
          for (const auto &it : sequences) {
  ...
  void Graph_Constructor::LoadFromPaf(std::vector<std::unique_ptr<biosoup::NucleicAcid>> &sequences,
    ...
    try {
      ...
      for (std::unique_ptr<Overlap> &overlap_ptr : overlaps) {
        ...
        Node *tail_node =
            tail_node_strand ? sequence_to_node[overlap.q_name].get() : sequence_to_node[overlap.q_name]->pair;
        Node *head_node =
            head_node_strand ? sequence_to_node[overlap.t_name].get() : sequence_to_node[overlap.t_name]->pair;
